...
async def assemble_logic(symbol, start_price, current_price):
    ...
    if hedging_states[symbol_name]['negative_hedge']:
        if direction == 'Down' and threshold_no <= 0.87:
            ...
            # close_trade(symbol_name, "buy", symbol['lot'])
    ...
    if hedging_states[symbol_name]['positive_hedge']:
        if direction == 'Up' and threshold_no >= -0.87:
            ...
            await close_trades_by_symbol(symbol)
            # close_trade(symbol_name, "sell", symbol['lot'])
    ...
    if threshold_no == 1:
        await trade_place(symbol_name, "sell", symbol['lot'], hedge=False)
    ...
    if 0.05 <= threshold_no <= 0.67:  # Expanded positive hedge range
        ...
        # place_trade(symbol_name, "sell", symbol['lot'])
        await trade_place(symbol_name, "sell", symbol['lot'], hedge=True)
        messages.append(
        ...
        await trade_place(symbol_name, "buy", symbol['lot'])
